...
export async function run(argv: readonly string[], cwd = process.cwd()): Promise<number> {
  ...
  if (command === "outcome") {
    const [action, id] = args;
    const usage = "Usage: ways outcome open <id> --goal=<text> --criterion=<ID>:<text>... [--memory=none|normal|high] [--evaluation=independent|self] | evaluate | remediate --reason=<text> | memory-review digest | memory-review submit <review.json> | close | cancel";
    if (action === "open" && id) {
      ...
      await openOutcome(cwd, id, requiredOption(args, "--goal"), criteria, memory as MemoryTier, evaluation as EvaluationMode);
      console.log(`Outcome ${id} opened with ${criteria.length} acceptance criteria, ${memory} memory assurance and ${evaluation} evaluation; execute through tasks, then run ways outcome evaluate.`);
      return 0;
    }
