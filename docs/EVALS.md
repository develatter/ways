...
- `compliance` is graded from the repository history after the recorded bootstrap revision. `compliant` (and `fullSddCompleted`, its alias) requires an SDD work named after the task certified through close and none of these issues: history-audit or integrity issues, commits of another work (`eval-foreign-work`), changes to `.ways/config.json`, the manifest, managed hooks, `scripts/check.sh` or `AGENTS.md` (`eval-harness-tampered`), downgrades, uncommitted changes (`git status`) or a still-active work. Ways' own memory and discovery commits are ordinary SDD evidence. The record also counts SDD works started/closed, remediation attempts and validation failures. If grading fails, or the task fails before grading, the functional grade is kept and compliance reports `eval-compliance-error`. For A/B compliance is `{ "applicable": false }`. The threat model is an honest agent that may skip or misuse the process; an agent that deliberately subverts Git (forged trailers with hooks disabled, rewritten history, replace refs) is beyond what local history proves, the same limit `ways check --history` documents.
